		...
		if (output.IsValid)
		{
			...
			if (!IsAutoOrienting())
			{
				if (AAIController* AIController = Cast<AAIController>(GetController()))
				{
					...
					FRotator const DesireRotation{ CurrentRotation + DeltaRotation };

					if (!FMath::IsNearlyEqual(CurrentRotation.Yaw, DesireRotation.Yaw))
					{
						AIController->SetControlRotation(DesireRotation);
						FaceRotation(DesireRotation);
					}
				}
			}
		}
